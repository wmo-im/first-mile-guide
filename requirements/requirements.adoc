...
* Hub - The software at the central server that receives data from the Node and prepares it for further processing. It must be able to receive the data usingthe agreed Data Format and Protocol.
* Protocol - The IP based set of rules that determine how data is transferred from the Node to the Hub.
...
* The First-Mile Data Collection protocol facilitates the transfer of data from the Node to the Hub.
* Communication is strictly unidirectional — data flows only from the Node to the Hub.
...
* The Protocol (communication transport layer) used is MQTT. MQTT serves as the message delivery Protocol for transmitting the Protobuf payloads from the Node to the Hub.
* For bandwidth-constrained environments (such as satellite links), the "First-Mile" is considered data transmission from the satellite ground base station to the Hub. The actual data transmitted from the datalogger through the satellite link to the ground station can use any suitable Data Format and Protocol. This Data Format should then be transcoded to the First-Mile Data Collection format when the satellite data is transfered from the satellite ground station to the Hub.

== Overview of the Protocol

The Node has some measurement data that it wants to send to the Hub. The Node encodes the measurement data into a Protobuf payload using the agreed/defined Data Schema. The Node then transmits this Protobuf payload over MQTT to the Hub.
...
The `Data` message is the main message that is sent from the Node to the Hub. It contains the following fields:
...
The `Metadata` message contains the site's configuration details, including the types of sensors installed and the measurement parameters along with their units. This message must be sent to the Hub at least once after each configuration update. It contains the following fields:
...
* Transmit the Protobuf payload over MQTT to the Hub.
