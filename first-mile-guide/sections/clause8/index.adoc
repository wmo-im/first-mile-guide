...
Parameters that are not covered by the CF standard name vocabulary but are common to first-mile observation deployments are governed by a centrally managed registry maintained by WMO. This registry is hosted at `https://codes.wmo.int` and is governed by SC-IMT using the WMO fast-track code registration mechanism.
