...
The communication follows a unidirectional pattern where data flows from the Node to the Hub. The Node may transmit messages in the following patterns:
...
The Hub can only intepret data correctly if it has received the relevant `Metadata` message that defines the devices and parameters being observed. Therefore, it is recommended to transmit the `Metadata` message at least once before transmitting any `Data` messages.
